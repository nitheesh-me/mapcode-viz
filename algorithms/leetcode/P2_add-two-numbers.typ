...
#{
  ...
  let rho = ((l1, l2)) => {
    ...
    let res = ()
    for i in range(0, n) {
      res.push((if i < l1.len() {next(l1, i)} else {none},
...
//   // linked list viz
  let I_h = ((l1, l2)) => {
    diagram(
      (l1.enumerate().map(((i, x)) => node((i, 0), [$#x$], stroke: 1pt))).intersperse(
        edge("-|>")
      ),
      (l2.enumerate().map(((i, x)) => node((i, 0.5), [$#x$], stroke: 1pt))).intersperse(
        edge("-|>")
      ),
    ...
    for i in range(0, x.len()) {
      ...
      if val_a != none {
        if diff_mask != none and diff_mask.at(i).at(0) {
          ...
          a.push(rect(stroke: none, inset: 2pt)[$#if val_a != none {head(val_a)} else {[$bot$]}$])
        }
      }
      if val_b != none {
        if diff_mask != none and diff_mask.at(i).at(1) {
          ...
          b.push(rect(stroke: none, inset: 2pt)[$#if val_b != none {head(val_b)} else {[$bot$]}$])
        }
      }
      if diff_mask != none and diff_mask.at(i).at(3) {
        c.push(rect(fill: yellow.transparentize(10%), inset: 2pt)[$#carry$])
      } else {
        c.push(rect(stroke: none, inset: 2pt)[$#carry$])
      }
      if diff_mask != none and diff_mask.at(i).at(2) {
        r.push(rect(fill: yellow.transparentize(10%), inset: 2pt)[$#res$])
      } else {
        r.push(rect(stroke: none, inset: 2pt)[#if res != none {res} else {[$bot$]}])
      }
    }
    diagram(
      (a.enumerate().map(((i, x)) => node((i, 0), [#x], stroke: 1pt, inset: 3pt))).intersperse(
        edge("-|>")
      ),
      (b.enumerate().map(((i, x)) => node((i, 0.5), [#x], stroke: 1pt, inset: 3pt))).intersperse(
        edge("-|>")
      ),
  ...
  let A_h = (a) => {
    diagram(
      (a.enumerate().map(((i, x)) => node((i, 0.5), [$#x$], stroke: 1pt))).intersperse(
        edge("-|>")
      ),
