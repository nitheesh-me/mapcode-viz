...
#let mapcode-viz(
  ...
  let grouped_nodes = xs.chunks(group-size).enumerate(start:1).map(((group_idx, group)) => {
    group.enumerate().map(((i, x_i)) => {
  ...
  let y_pos_fix = if (calc.rem(xs.len(), group-size) in (0, 1)) {calc.div-euclid(xs.len(), group-size)} else {calc.div-euclid(xs.len(), group-size) +  1}
  if group-size == 1 {
    y_pos_fix = xs.len() - 1
  }

  box(scale(scale-fig, reflow: true)[#diagram(..args,
